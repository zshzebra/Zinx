...
pub fn setMast(irq_num: u8) void {
    const port: u16 = if (irq_num < 8) MASTER_DATA_REG else SLAVE_DATA_REG;
    const shift = @as(u8, @intCast(irq_num % 8));
...
pub fn init() void {
    log.info("init pic", .{});
    defer log.info("initialised pic", .{});
