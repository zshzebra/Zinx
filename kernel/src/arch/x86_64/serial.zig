...
pub fn computeLcrValue(char_len: u8, stop_bit: bool, parity_bit: bool, msb: u1) u8 {
    if (char_len < 5 or char_len > 8)
        return error.InvalidChararacterLength;

    return (char_len - 1) & 0x3 |
...
fn baudrateDivisor(baud: u32) SerialError!u16 {
    ...
    return @as(u8, @truncate(MAX_BAUDRATE / baud));
}
